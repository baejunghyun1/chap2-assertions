...
public class JupiterAssertionsTests {
    ...
    * 모든 Jupiter Assertions의 메소드는 정적(static) 메소드 형태로 제공하고 있다.
    ...
    //when : 테스트할 메소드를 호출한다.
    //then : 실행 결과를 검증한다.
    // 하나의 테스트 메소드에는 한가지를 검증할수있도록 작성하는게 좋다
    // 테스트 할 메소드와 검증을 동시에 하는 경우도 있다. (when & then)
...
//        Assertions.assertEquals(expected, result, 1);
        ...
        // 3번째 인자로 문자열로 테스트 실패 시 보여줄 메세지를 작성할 수 있다.
//        Assertions.assertEquals(expected, result, "실패할 때 보여줄 메세지");
    ...
    void testAssertNotEqualsWithInstances() {
        ...
        //then
        // 단, equals 와 hashcode 가 오버라이딩 되어 있는 경우는 같다고 판단
    ...
    /* 목차. 5. assertTrue */
    /* 필기. assertTrue(actual) 메소드는 결과값이 true 인지 확인하는 메소드 */
    @Test
    ...
    void testAssertTrue() {
        ...
        //then
        /*assertEquals로 true 값과 일치하는지를 확인하는 기능과 동일하다. */
        Assertions.assertEquals(true, result);
//        Assertions.assertTrue(result);
    }

    /* 목차. 6. assertFalse */
    /* 필기. assertFalse(actual) 메소드는 결과값이 false 인지 확인한다.*/
    @Test
    ...
    /* 목차. 7. assertAll */
    /* 필기. assertAll(Excutable....) 모든 Assertion이 실행되고 실패가 함께 보고되는 그룹화된 Assertion */
    @Test
    ...
    void testAssertAll() {
        ...
        //then
        Assertions.assertAll("그룹화 된 테스트의 이름( 테스트 실패시 보여짐 )",
                () -> Assertions.assertEquals("길길", person.getFirstName(), () -> "firstName 이 잘못됨"),
